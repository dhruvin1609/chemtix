...
<section class="content">
    ...
    <div class="container-fluid">
        <form action="" name="createCustomerProductForm" id="createCustomerProductForm" method="POST">
            <div class="card">
                <div class="card-body">								
                    <div class="row">
                        <div class="col-md-6">
...
@section('customJs')
<script>
    $('#createCustomerProductForm').submit(function(event){
       ...
       $("button[type=submit]").prop('disabled',true)
        $.ajax({
            url:'{{route("customer_product.store")}}',
            type:'post',           
      ...
      $('#customerCSVupload').submit(function(event){
        event.preventDefault();
        var form = $(this)[0];
        var data = new FormData(form);
       var element = $(this);
       $("button[type=submit]").prop('disabled',true)
        $.ajax({
            url:'{{route("customer_product.import")}}',
            type:'post',
            ...
            success : function(res){
                ...
                if(errors['cas_number']){
                    $('#file').addClass('is-invalid').siblings('p').addClass('invalid-feedback').html(errors['cas_number'])
                } else{
                    $('#file').removeClass('is-invalid').siblings('p').removeClass('invalid-feedback').html("")
                }
